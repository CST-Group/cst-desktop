...
package br.unicamp.cst.util.motivational;
...
public abstract class MotivationalCodelet extends Codelet {
    ...
    private br.unicamp.cst.motivational.Drive outputDrive;
    ...
    public MotivationalCodelet(String id, double level, double priority, double urgencyThreshold) {
        ...
        setOutputDrive(new br.unicamp.cst.motivational.Drive(getId(),
                0,
                getPriority(),
                getLevel(),
                getUrgencyThreshold(),
        ...
        synchronized (this) {
            try {
                if (getDrivesRelevance().size() > 0) {
                    List<br.unicamp.cst.motivational.Drive> listOfDrives = new ArrayList<br.unicamp.cst.motivational.Drive>();

                    for (Map.Entry<Memory, Double> driveMO : getDrivesRelevance().entrySet()) {

                        br.unicamp.cst.motivational.Drive drive = (br.unicamp.cst.motivational.Drive) driveMO.getKey();

                        br.unicamp.cst.motivational.Drive driveClone = new br.unicamp.cst.motivational.Drive(drive.getName(), drive.getActivation(), drive.getPriority(), drive.getLevel(),
    ...
    private synchronized double verifingUrgencyThreshold(double driveActivation, br.unicamp.cst.motivational.Drive drive) {
    ...
    public abstract double calculateSecundaryDriveActivation(List<Memory> sensors, List<br.unicamp.cst.motivational.Drive> listOfDrives);
    ...
    private br.unicamp.cst.motivational.Drive getOutputDrive() {
        return outputDrive;
    }
