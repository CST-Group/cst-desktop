...
public abstract class MoodCodelet extends Codelet {
    ...
    private br.unicamp.cst.motivational.Appraisal appraisal;
    ...
    public void accessMemoryObjects() {
        ...
        if(getInputAppraisalMO() == null){
            ...
            setAppraisal((br.unicamp.cst.motivational.Appraisal) ((Memory) getInputAppraisalMO().getI()).getI());
        }
    ...
    public synchronized void calculateActivation() {
        List<br.unicamp.cst.motivational.Drive> listOfDrive  = new ArrayList<>();

        getListOfDrivesMO().stream().forEach(memory -> {
            listOfDrive.add((br.unicamp.cst.motivational.Drive) memory.getI());
        });

        Mood mood = moodGeneration(listOfDrive, (br.unicamp.cst.motivational.Appraisal) ((Memory) getInputAppraisalMO().getI()).getI(), getSensors());
        mood.setName(getId());
        setOutputMood(mood);
    }
    ...
    public abstract Mood moodGeneration(List<Drive> listOfDrives, br.unicamp.cst.motivational.Appraisal appraisal, List<Object> sensors);
    ...
    public br.unicamp.cst.motivational.Appraisal getAppraisal() {
        return appraisal;
    }
